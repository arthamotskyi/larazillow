    ...
    <Box>
        ...
        <form @submit.prevent="upload">
            <section class=" flex items-center gap-2 my-4">
                ...
                <button type="submit" class="btn-outline cursor-pointer disabled:opacity-25 disabled:cursor-not-allowed" :disabled="!canUpload">
                    Upload
                </button>
        ...
        <section class="mt-4 grid grid-cols-3 gap-4">
            <div v-for="image in listing.images" :key="image.id" class="flex flex-col justify-between">
                ...
                <Link :href="route('realtor.listing.image.destroy', { listing: props.listing.id, image: image.id })"
                        method="delete" as="button" class="mt-2 btn-outline text-xs cursor-pointer">
                    Delete
                </Link>
            </div>
        </section>
